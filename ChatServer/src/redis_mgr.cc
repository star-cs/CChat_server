...
namespace core
{
    RedisConnPool::RedisConnPool(std::size_t pool_size, const char *host, unsigned short port, const char *passwd)
    ...
    RedisMgr::RedisMgr()
    {
        ...
        _redisConnPool.reset(new RedisConnPool(
            boost::lexical_cast<std::size_t>(redisConnPoolNum),
            host.c_str(),
            boost::lexical_cast<unsigned short>(port),
            pwd.c_str()));

        // 安全日志：隐藏密码
        LOG_INFO("RedisMgr host: {}, port: {}, pwd: {}", host, port, (pwd.empty() ? "[empty]" : "******"));
    }
