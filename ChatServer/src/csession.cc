 ...
 * @LastEditTime: 2025-06-22 14:38:01
...
namespace core
{
    ...
    void CSession::Close()
    {
        ...
        std::string user_str = std::to_string(_user_id);

        // Reids 里 清除，ubaseinfo
        std::string userkey = USER_BASE_INFO + user_str;
        RedisMgr::GetInstance()->Del(userkey);

        // Redis 里 清除，utoken
        std::string tokenkey = USERTOKENPREFIX + user_str;
        RedisMgr::GetInstance()->Del(tokenkey);

        // Redis 里 清除，用户所在 ChatServer
        std::string ipkey = USERIPPREFIX + user_str;
        RedisMgr::GetInstance()->Del(ipkey);

        // 断掉 UserMgr里记录着， uid - sessionId 的关系
        ...
        boost::asio::async_read(
            ...
            [this, self, length](const boost::system::error_code &ec, std::size_t bytes_read)
            {
                try
                {
                    ...
                    {
                        LOG_ERROR("read length not match, read [{}] , total [{}] ", bytes_read, HEAD_TOTAL_LEN);
                        Close();
                        _server->ClearSession(_session_id);
                    }
    ...
    void CSession::AsyncReadBody(int total_len)
    {
        ...
        boost::asio::async_read(
            _socket,
            boost::asio::buffer(_data, total_len),
            boost::asio::transfer_exactly(total_len),
            [this, self, total_len](const boost::system::error_code &ec, std::size_t bytes_read)
            {
                try
                {
                    ...
                    ::memcpy(_recv_msg_node->_data, _data, bytes_read); // 客户端端发送过来的消息是保持了原始字节序
                    _recv_msg_node->_cur_len += bytes_read;
                    _recv_msg_node->_data[_recv_msg_node->_total_len] = '\0'; // 方便识别到 \0 字符串进行截断
